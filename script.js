...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLanguage(lang) {
        ...
        // --- メニューカテゴリボタンのテキストを更新するロジックを修正・追加 ---
        dom.menuButtons.forEach(button => {
            const categoryKey = button.dataset.key; // HTMLで data-key="japaneseCategory" などと設定済み
            if (translations[lang] && translations[lang][categoryKey]) {
                button.textContent = translations[lang][categoryKey];
            } else {
                // キーが見つからない場合のフォールバック（例: 英語）
                button.textContent = translations['en'][categoryKey] || button.textContent;
            }
        });
        // --- ここまで修正・追加 ---
            ...
            card.innerHTML = `
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex justify-between items-center mt-auto">
                        ...
                        <button data-item-id="${item.id}" class="add-to-cart-button bg-green-100 text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-200 transition">追加 <i class="fas fa-plus ml-1"></i></button>
                    </div>
                </div>
            `;
    ...
    updateLanguage(savedLang); // ここでバナーのテキストとハイライトが初期化されます
    ...
    dom.menuButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // すべてのメニューボタンから、アクティブ状態を示すクラスを削除します
            dom.menuButtons.forEach(btn => {
                btn.classList.remove('active-category-button'); // CSSで定義した共通のアクティブクラス
            });
            ...
            // クリックされたボタンに、アクティブ状態を示す共通のクラスを追加します
    ...
    // ページロード時の初期表示として「日本料理」をアクティブにする
    // `updateLanguage` 関数が最初に呼び出されるときに、バナーのテキストも初期化されます
    const initialButton = document.querySelector('.menu-button[data-menu="japanese"]');
    if (initialButton) {
        initialButton.click(); // クリックイベントをトリガーして初期表示とアクティブスタイル適用
    }
});
